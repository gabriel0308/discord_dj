...
class VibeCog(commands.Cog):
    ...
    async def get_played_songs(self, guild_id: int, limit: int = 100) -> List[dict]:
        def _fetch():
            conn = sqlite3.connect(str(self._db_path))
    ...
    async def count_played_songs(self, guild_id: int) -> int:
        def _count():
            conn = sqlite3.connect(str(self._db_path))
    ...
    async def _auto_extend(self, guild_id: int, state: dict):
        ...
        history = await self.get_played_songs(guild_id, limit=100)
    ...
    async def vibe_play(self, ctx: commands.Context, *, query: str):
        ...
        history = await self.get_played_songs(ctx.guild.id, limit=100)
        songs = None
        try:
    ...
    async def vibe_status(self, ctx: commands.Context):
        ...
        msg = f"**Vibe Session:**\n"
        msg += f"Total songs in history: `{total_db}`\n"
    ...
    async def generate_playlist(self, vibe: str, history: list = None) -> list:
        ...
        system_prompt = (
            ...
            "3. None of the songs should match the already-played list above.\n"
